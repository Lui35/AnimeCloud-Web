...
export function getLibrary(userID: string, includeDeleted = false): LibraryItem[] {
  ...
  return (db.prepare(query).all(userID) as Record<string, unknown>[]).map((row) => ({ animeId: String(row.anime_id), name: String(row.name), image: String(row.image), year: String(row.year), category: Number(row.category), deleted: Boolean(row.deleted), updatedAt: Number(row.updated_at) }));
}

export function putLibraryItem(userID: string, item: Omit<LibraryItem, "updatedAt">, updatedAt = Date.now()) {
  db.prepare(`INSERT INTO library_items (user_id, anime_id, name, image, year, category, deleted, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
export function putProgress(userID: string, value: { episodeId: string; animeId?: string; episodeNumber?: number | null; position: number; duration: number; watched: boolean }, updatedAt = Date.now()) {
  ...
  db.prepare(`INSERT INTO episode_progress (user_id, episode_id, anime_id, episode_number, position, duration, watched, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
export function getSyncState(userID: string) {
  return db.prepare("SELECT * FROM sync_state WHERE user_id = ?").get(userID) as Record<string, unknown> | undefined;
}
...
export function updateSyncState(userID: string, fields: { legacyBaselineAt?: number; anilistBaselineAt?: number; lastLegacySyncAt?: number; lastAnilistSyncAt?: number }) {
  db.prepare(`INSERT INTO sync_state (user_id, legacy_baseline_at, anilist_baseline_at, last_legacy_sync_at, last_anilist_sync_at)
    VALUES (?, ?, ?, ?, ?)
...
export function saveAniListConnection(userID: string, encryptedToken: string, viewerID: number, viewerName: string) {
  db.prepare(`INSERT INTO anilist_connections (user_id, encrypted_token, viewer_id, viewer_name, connected_at) VALUES (?, ?, ?, ?, ?)
...
export function getAniListConnection(userID: string) {
  return db.prepare("SELECT * FROM anilist_connections WHERE user_id = ?").get(userID) as Record<string, unknown> | undefined;
}

export function removeAniListConnection(userID: string) {
  db.prepare("DELETE FROM anilist_connections WHERE user_id = ?").run(userID);
}
