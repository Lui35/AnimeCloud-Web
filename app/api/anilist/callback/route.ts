...
import { saveAniListConnection } from "../../_db";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const clientID = process.env.ANILIST_CLIENT_ID, clientSecret = process.env.ANILIST_CLIENT_SECRET;
    if (!clientID || !clientSecret) throw new Error("AniList OAuth is not configured.");
    const redirectURI = process.env.ANILIST_REDIRECT_URI || `${url.origin}/api/anilist/callback`;
    ...
    return Response.redirect(new URL("/?anilist=connected", url.origin));
